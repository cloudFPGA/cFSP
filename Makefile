# This helps with creating local virtual environments, 
# and uploading trieres packages to PyPI.
#
...
# Project settings
PKGDIR := $(or $(PACKAGE), ./)
REQUIREMENTS := $(shell find ./ -name $(REQUIRE) ! -path "./cFp_Vitis/*")
...
upload-dirty:
	$(PYTHON) -m twine upload  dist/$(PACKAGE)-$(shell $(PYTHON) setup.py --version)*

upload: .git-no-changes register upload-dirty
